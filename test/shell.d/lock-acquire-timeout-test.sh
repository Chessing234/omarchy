...
assert(
  /id: lockAcquireTimeoutTimer[\s\S]*interval: 5000[\s\S]*lock-failed: acquire-timeout/.test(serviceQml),
  'a pending lock that never reaches the compositor times out inside the sleep-lock budget'
)
...
assert(
  !/if \(root\.lockRequested\) \{[\s\S]*lockAcquireTimeoutTimer\.restart\(\)/.test(serviceQml),
  're-queue while lockRequested does not extend the fail-closed deadline'
)
...
assert(
  /readonly property bool locked: lockRequested \|\| sessionLock\.locked \|\| sessionLock\.secure/.test(serviceQml),
  'locked still includes lockRequested so idle screensaver gating is unchanged'
)
JS
